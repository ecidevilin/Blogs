...
* 因为预先不知道去重复的id有多少，所以显存只能饱和式申请，CompactNodeDataBuffer和NodeDataBuffer的成员数量是Resolution.x\*Resolution.y\*MSAACount，显存分分钟爆炸（其实也还好），然而我并没有发现CompactNodeDataBuffer有什么存在的必要，去重复的时候，完全可以把数据保存到NodeDataBuffer里。   
...
* Deep Opacity Maps
* [TressFX](https://gpuopen.com/gaming-product/tressfx/)